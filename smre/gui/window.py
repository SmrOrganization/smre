...
def begin_window(
    ...
    default_position = Vec2(style.scaled(60.0 + slot * 24.0), style.scaled(60.0 + slot * 24.0))
    default_size = Vec2(style.scaled(320.0), style.scaled(240.0))
    ...
    if decorated:
        ...
        close_size = style.scaled(16.0)
        margin = style.scaled(6.0)
        close_rect = (
            window.position.x + window.size.x - close_size - margin,
            window.position.y + (title_bar_height - close_size) * 0.5,
            close_size, close_size,
        )

        collapse_size = style.scaled(16.0)
        collapse_rect = (
            window.position.x + margin,
    ...
    edge_margin = style.scaled(40.0)
    window.position.x = max(-window.size.x + edge_margin, min(window.position.x, context.io.display_size.x - edge_margin))
    ...
    if decorated:
        ...
        if context.font is not None:
            ...
            draw_list.add_text(context.font, collapse_glyph, Vec2(collapse_rect[0] + glyph_inset, collapse_rect[1]), style.text)
            if closable:
                close_color = style.button_active if hovering_close else style.text
                draw_list.add_text(context.font, "x", Vec2(close_rect[0] + glyph_inset, close_rect[1]), close_color)
...
def _resize_grip(context, window):
    ...
    if context.is_active(resize_id) and context.io.mouse_down[0]:
        delta = Vec2(mouse.x - context._drag_anchor_mouse.x, mouse.y - context._drag_anchor_mouse.y)
        new_width = max(style.scaled(120.0), context._drag_anchor_value.x + delta.x)
        new_height = max(style.scaled(80.0), context._drag_anchor_value.y + delta.y)
...
def _draw_scrollbar(context, window, title_bar_height):
    style = context.style
    track_x = window.position.x + window.size.x - style.scrollbar_size - style.scaled(2.0)
    ...
    thumb_h = max(style.scaled(20.0), track_h * visible_ratio)
    ...
    hover_t = context.animate(scrollbar_id + "::hover", 1.0 if hovering_thumb else 0.0)
    if context.is_active(scrollbar_id):
        color = style.scrollbar_grab_active
    else:
        color = style.scrollbar_grab.lerp(style.scrollbar_grab_hovered, hover_t)
    context.draw_list.add_rect_filled(Vec2(track_x, thumb_y), Vec2(style.scrollbar_size, thumb_h), color, rounding=style.scaled(3.0))
